let matrix = randomize(50, 50);  //Randomizes initial state
...
document.addEventListener('DOMContentLoaded', function(){  //Loads the DOM
	...
	function populate(){  //Places populate() into memory
		...
		for (i = 0; i < matrix.length; i++){  //Iterates through first tier array
			...
			for (j = 0; j < matrix[i].length; j++){  //Iterates through second tier array
				...
				function existenceState(){  //Places existenceState into memory
					if (matrix[i][j] === 1){  //Sets up conditions for classes
			 	...
			 	row.appendChild(data);  //Appends tr with td
			  	...
			  	existenceState();  //Executes existenceState to apply classes
			}
		};
	}
	...
	document.querySelector('#populate').addEventListener('click', function(){
		...
		document.querySelector('#run').addEventListener('click', function(){  //Adds event listener for Run button
			matrix = populateNextState(matrix);
		...
		document.querySelector('#stop').addEventListener('click', function(){  //Adds event listener for Stop button
			matrix = populateNextState(matrix);
		});
	})
